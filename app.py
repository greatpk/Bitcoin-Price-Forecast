...
#mysql = MySQL(app)
mydb=pymysql.connect(host='127.0.0.1', user='root', password='root', port=3306, database='logindb')
...
def login():
	...
	if request.method == 'POST' and 'username' in request.form and 'password' in request.form:
		...
		print(sql)
		logindt = pd.read_sql_query(sql, mydb)
		pwd=logindt['username'].values
		print(pwd)
		my_string = str(pwd[0])
		if username==my_string:
			session['loggedin'] = True
			id1=logindt['id'].values
			session['id'] = str(id1[0])
			uname=logindt['username'].values
			session['username'] = str(uname[0])
...
def predict():
	...
	model=pickle.load(open("model_"+str(day)+".sav", 'rb'))
	...
	for i in days:
		if(i==day):
			...
			model=pickle.load(open("model_"+str(i)+".sav", 'rb'))
